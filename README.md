...
9. The PlayerManager has `onPlayerConnectedCallbacks` that you can listen to for new players.
10. That callback provides a `playerInstance`. You can use that `playerInstance` to push or pop input contexts, using `playerInstance.inputManager:pushInputContext(inputContextName)` and `playerInstance.inputManager:popInputContext(inputContextName)`
